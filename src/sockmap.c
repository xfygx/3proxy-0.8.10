...
int sockmap(struct clientparam * param, int timeo){
 ...
 debug_print("sockmap() begin. srv buf size %d\n", SRVBUFSIZE);
...
(*param->srv->logfunc)(param, "Starting sockets mapping");
 ...
 if(!param->waitclient64){
 	/* 1024 is to fix crash issue due to inject vector into html. */
	if(!param->srvbuf && (!(param->srvbuf=myalloc(bufsize+1024)) || !(param->srvbufsize = bufsize))){
		return (21);
	}
 	debug_print("sockmap() malloc memory for srvbuf. %p\n", param->srvbuf);
 }
 ...
 debug_print("handlepredatflt() call in sockmap()\n");
 ...
 if(!param->nolongdatfilter){
	if(param->cliinbuf > param->clioffset){
		debug_print("handledatfltcli() call in sockmap()\n");
	...
	if(param->srvinbuf > param->srvoffset){
		debug_print("handledatfltsrv() call in sockmap()\n");
		...
		if(action != PASS) { debug_print("bbb\n"); return 19; }
	}
 }
...
(*param->srv->logfunc)(param, "send to client");
		...
		debug_print("sendto() call in socketmap(). waitserver64:%lld, received:%lld\n", param->waitserver64, received);
		// debug_print("sendto len : %d\n", (!param->waitserver64 || (param->waitserver64 - received) > (param->srvinbuf - param->srvoffset)) );
		// debug_print("sendto len : %d\n",  param->srvinbuf  );
		// debug_print("sendto len : %d\n",   param->srvoffset );		
		// debug_print("sendto len : %d\n",  param->srvinbuf - param->srvoffset );
		// debug_print("sendto len : %d\n",  (int)(param->waitserver64 - received));				
		// debug_print("sendto len : %d\n", (!param->waitserver64 || (param->waitserver64 - received) > (param->srvinbuf - param->srvoffset))? param->srvinbuf - param->srvoffset : (int)(param->waitserver64 - received));
...
(*param->srv->logfunc)(param, "send to server");
		...
		debug_print("sendto remsock, %lld\n", param->waitclient64);
...
(*param->srv->logfunc)(param, "recv from client");
		...
		if (res==0) {
			...
			param->cliinbuf += res;
			if(!param->nolongdatfilter){
				debug_print("call handledatfltcli in sockmap 222\n");
...
(*param->srv->logfunc)(param, "recv from server");
		...
		if (res==0) {
			...
			if(!param->nolongdatfilter){
				debug_print("handledatfltsrv() call in sockmap() after recvfrom()\n");
