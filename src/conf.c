...
#endif
PLUGINAPI int ssl_plugin (struct pluginlink * pluginlink, int argc, char** argv);
...
int start_proxy_thread(struct child * chp){
  ...
  printf("enter start_proxy_thread\n");
#ifdef _WIN32
  HANDLE h;
#endif
...
static int h_include(int argc, unsigned char **argv){
	debug_print("Don't support include\n");
	return 0;
}
...
static int h_plugin(int argc, unsigned char **argv){
	...
	printf("enter h_plugin() found %s in %s\n", argv[1], argv[2]);
	return ssl_plugin(&pluginlink, argc - 2, (char **)argv + 2);
...
int readconfig(void){
 ...
 unsigned char * buf = NULL;
  int bufsize = STRINGBUF;
  int inbuf = 0;
  int argc;
  struct commands * cm;
  int res = 0;

  #define conf_linenumber 10
  char conf[][256] = {
	...
	"auth none",
	"internal 127.0.0.1",
	"plugin SSLPlugin.dll ssl_plugin",
	"ssl_certcache .\\",
	"ssl_mitm",
  ...
  for (linenum = 1; linenum <= conf_linenumber; linenum++){

	memset(buf, 0, bufsize);
	strcpy(buf, conf[linenum-1]);
